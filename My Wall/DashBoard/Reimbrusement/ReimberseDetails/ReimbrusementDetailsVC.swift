...
class ReimbrusementDetailsVC: UIViewController {
    
    
    var dashBoardType : DashBoardType = .Grievances
    var eachBlog : ReimbrusementModelElement? = nil

    var ReimbursementObj : ReimbursementDetailsModel? = nil
...
//        }
        ...
        if self.dashBoardType == .Reimbursement {
             catUrl  =  myHelper.reimbursementDetails + userID + "/" + "Reimbursement"
        }
        ...
        ApiService.sharedManager.getServiceWithoutToken(catUrl, parameters) { (urlData) in
            ...
            do {
                let rootDicnew = try JSONDecoder().decode(ReimbursementDetailsModel.self, from: urlData)
...
//                    let errorModel = try JSONDecoder().decode(ErrorModel.self, from: urlData)
                    self.view.makeToast("View Reimbursement - 409")
                }
                catch {
...
extension ReimbrusementDetailsVC: UITableViewDelegate, UITableViewDataSource,UIScrollViewDelegate
{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            if self.ReimbursementObj != nil {
//                cell.textView.text = self.ReimbursementObj?.cmrfRemarks.first?.remark
...
struct ReimbursementDetailsModel: Codable {
    let reimbursement: Reimbursement
    let sanctionedDetails: [SanctionedDetail]
    let statuses: [StatusObj]
    let districts: [District]
    let mandals: [Mandal]
    let villages: [Village]
    let fileIdxNo: Int
    let uploadedFiles: [UploadedFileArray]?
    let sanctionedFiles: [JSONAny]
    let cmrfRemarks: [CmrfRemark]
}

// MARK: - CmrfRemark
struct CmrfRemark: Codable {
    let id, remark, remarkBy, remarkDate: String
    let remarkForObject, objID, loggedInUser: String

    enum CodingKeys: String, CodingKey {
        case id, remark, remarkBy, remarkDate, remarkForObject
        case objID = "objId"
        case loggedInUser
    }
}
...
struct SanctionedDetail: Codable {
    let id: String
    let sanctionedAmt: Int
    let sactinedAmtInWords, sanctionedDate, sanctionedNo, chequeNo: String
...
// MARK: - UploadedFile
struct UploadedFileArray: Codable {
...
struct Reimbursement: Codable {
    let id, fileNo, cmrfType, status: String?
    let date, applicationDate, refBy, refPhone: String?
    let salutation, patientName, sex, relation: String?
    let relativeName, pPhone, altPhone, aadhar: String?
    let age, pAddress, hamlet, village: String?
    let mandal, district, state, country: String?
    let pincode, desease, hospitalName, hospAddress: String?
    let additionalRequest: Bool
    let previousSanctionedAmt: Int
    let previousAmtInWords: String?
    let treatmentAmount: Int
    let amtInWords, peshiLrNo, peshiDate, remarks: String?
